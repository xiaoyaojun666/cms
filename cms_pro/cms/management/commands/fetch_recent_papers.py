...
class Command(BaseCommand):
    help = 'Fetches papers from arXiv from the last specified days and saves them into the database'

    def add_arguments(self, parser):
        parser.add_argument('days', type=int, help='Number of days to go back for fetching papers')
        ...
        if response.status_code == 200:
            ...
            for entry in entries:
                ...
                if published_date >= start_date:
                    ...
                    self.stdout.write(f"非指定日期的论文{published_date_text}\n")
        else:
